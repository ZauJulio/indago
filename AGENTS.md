...
- `packages/HyperDown` (`@virtus/hyper-down`) — Markdown/MDX CMS: frontmatter parser,
...
- `packages/HyperJson` (`@virtus/hyper-json`) — typed JSON content: schema validation +
  TypeScript codegen (parallel pool, `HYPERJSON_CONCURRENCY`) + headless hooks.
- `packages/scaffold` (`create-virtus-app`) — scaffolder CLI: 4 templates (vike /
